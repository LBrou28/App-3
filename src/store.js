window.Reel = (() => {
 ...
 let state=defaults();
 try {const s=JSON.parse(localStorage.getItem(key));if(s && Array.isArray(s.people)&&s.people.length===3&&s.people.every(p=>p&&typeof p.name==='string'&&Array.isArray(p.genres)&&p.genres.every(g=>genres.includes(g))&&[110,130,150,180].includes(p.max))&&Array.isArray(s.shortlist)&&s.shortlist.every(id=>movies.some(m=>m.id===id))&&Array.isArray(s.votes)&&s.votes.length===3&&s.votes.every(v=>v===null||s.shortlist.includes(v))) state=s;}catch(_){}
